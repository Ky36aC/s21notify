...
Прод — LXC `ssh root@s21notify.lan` (Debian, 512 МБ), systemd-сервис `s21notify-v3`,
каталог `/opt/s21notify-v3`. **Компилировать на LXC нельзя** (памяти нет) — собирает
...
ssh root@s21notify.lan "journalctl -u s21notify-v3 -n 50 --no-pager"
```
