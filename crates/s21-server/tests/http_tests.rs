...
fn test_config() -> AppConfig {
    ...
    AppConfig {
        ...
        public_url: "https://s21notify.tobitrix.ru".into(),
...
async fn регистрация_перелогин_и_второй_мессенджер() {
    ...
    let (status, body) = call(
        ...
        post_json(
            "/api/credentials",
            json!({"login": "floriato", "password": "wrong"}),
            Some(&tg_jwt),
        ),
    )
    .await;
    ...
    let (status, body) = call(
        ...
        post_json(
            "/api/credentials",
            json!({"login": "Floriato@student.21-school.ru", "password": "verysecret"}),
            Some(&tg_jwt),
        ),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "{body}");
    assert_eq!(body["s21_login"], "floriato");
    ...
    assert_eq!(me["s21_login"], "floriato");
    ...
    let (status, _) = call(
        ...
        post_json(
            "/api/credentials",
            json!({"login": "floriato", "password": "verysecret"}),
            Some(&max_jwt),
        ),
    )
    .await;
