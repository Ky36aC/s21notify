...
async fn привязки_и_каскадное_удаление() {
    let p = pool().await;
    let id = db::create_user(&p, "floriato", b"enc").await.unwrap();
    db::link_account(&p, id, "telegram", "111", "111", Some("floriato_tg"))
        .await
        .unwrap();
    db::link_account(&p, id, "max", "222", "chat222", None)
        .await
        .unwrap();

    let accs = db::active_accounts(&p, id).await.unwrap();
    assert_eq!(accs.len(), 2);

    // блокировка гасит доставку
    db::set_account_status(&p, "telegram", "111", "blocked")
        .await
        .unwrap();
    assert_eq!(db::active_accounts(&p, id).await.unwrap().len(), 1);

    // повторный /start того же юзера возвращает active
    db::link_account(&p, id, "telegram", "111", "111", Some("floriato_tg"))
        .await
        .unwrap();
...
async fn перепривязка_мессенджера_к_другому_аккаунту() {
    ...
    db::link_account(&p, u1, "telegram", "111", "111", None)
        .await
        .unwrap();
    // тот же телеграм входит под другим s21-логином → привязка переезжает
    db::link_account(&p, u2, "telegram", "111", "111", None)
        .await
        .unwrap();
