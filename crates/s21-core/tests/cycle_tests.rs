...
fn роль_проверяющего_и_онлайн() {
    let mut b = booking("b1", "2026-07-20T09:30:00Z", "floriato", "victim");
    b.info.online = true;
    let line = fmt_booking_line(&b.info, "floriato");
    assert_eq!(
        line,
...
fn тумблеры_гасят_свои_события() {
    ...
    let out = run_cycle(
        &prev,
        &fetched_bookings(vec![
            booking("b2", "2026-07-20T10:30:00Z", "peer", "floriato"), // новая
        ]),
        &s,
        "floriato",
        now(),
...
fn alarm_message_текст() {
    let b = booking("b1", "2026-07-17T10:00:30Z", "peer", "floriato");
    assert_eq!(
        alarm_message(&b.info, "floriato", 30),
