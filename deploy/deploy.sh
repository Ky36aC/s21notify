#!/usr/bin/env bash
# Деплой v3 на LXC с машины разработчика (компиляция на LXC — никогда, там 512 МБ).
# Использование: ./deploy/deploy.sh [run_id]
#   без аргумента — последний успешный run workflow build на ветке v3
set -euo pipefail

HOST=root@s21notify.lan
DIR=/opt/s21notify-v3
ARTIFACT=s21notify-v3
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
...
if [ $# -ge 1 ]; then
    ...
    RUN_ID=$(gh run list --workflow build.yml --branch v3 --status success --limit 1 \
...
scp "$TMP/s21notify-v3.service" "$HOST:/etc/systemd/system/s21notify-v3.service"
...
echo "==> Рестарт"
ssh "$HOST" "systemctl daemon-reload && systemctl enable --now s21notify-v3 && systemctl restart s21notify-v3 && sleep 3 && systemctl is-active s21notify-v3"
