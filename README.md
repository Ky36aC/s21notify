...
Модель веток — **trunk-based** (минимум рутины для одного мейнтейнера):

- Работаем в `main`; для заметных изменений — короткая фича-ветка → PR → `main`.
- Перед merge проверяем в **локальном режиме** (`APP_MODE=local`, см. ниже) — это
  и есть тест-окружение, отдельный сервер не нужен.
- CI (`.github/workflows/build.yml`) гоняет тесты и сборку на каждый push в `main`
  и на каждый PR.
- Релиз — тег `vX.Y.Z` (см. [«Релизы»](#релизы)).

Задачи и дорожная карта — в [Issues](https://github.com/Ky36aC/s21notify/issues)
и [Projects](https://github.com/Ky36aC/s21notify/projects). Однократная настройка
доски-роадмапа (нужен gh со scope `project`):

```sh
gh auth refresh -s project
sh scripts/setup-github-project.sh   # создаст проект и добавит открытые issue
```
